...
```{r prevalence}
df <- read.csv("user-charsn5s5.csv")
names(df)
...
```{r link-stats-n5s5, fig.height=10}

user_df<-read.csv("user-charsn5s5.csv") 
...
```{r link-plot-n5s5, fig.height = 10}
...
df<-read.csv("users_formatch_n5s5.csv") %>%
...
att1 <- att_calc("Anti-vaccine", "h1")
att2 <- att_calc("Pro-vaccine", "h2")
