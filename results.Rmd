...
do_ztest <- function(filter_to, x_var){
  ...
  if(filter_to %in% c("Anti-vaccine", "Pro-vaccine")){
    ztest_x <- link_df %>% filter(Community==filter_to) %>% select(x_var) %>% unlist()
    ztest_n <- link_df %>% filter(Community==filter_to) %>% select(n_var) %>%  unlist()
  ...
  # for within-expertise comparison, subset to that perceived non-experts
    ...
    ztest_x <- link_df %>% filter(is_PE==filter_to) %>% select(x_var) %>% unlist()
    ztest_n <- link_df %>% filter(is_PE==filter_to) %>% select(n_var) %>% unlist()
...
z_table <- c()
for(my_filter in c("Anti-vaccine", "Pro-vaccine", "Not \nPerceived \nExpert", "Perceived \nExpert" )){
...
```{r link-plot, fig.height = 10}
link_df %>% select(c(Community, is_PE, starts_with("est.") | starts_with("user")))
...
for(my_metric in c("Betweenness", "Eigenvector", "Degree")){
...
cent_values %>% select(p.val, Community, ranking, metric)
...
bridge_values %>% select(p.val, Community, ranking, metric)
...
att_calc<-function(my_comm, hyp_name, add_covar=c()){ 
  ...
  print(love.plot(m.out, binary="std", abs=TRUE, thresholds=c(m=.1), var.order="unadjusted", 
            var.names = sapply(names(m.out$X), function(x) covar_names %>% filter(var==x) %>% select(name) %>% as.character),
            drop.distance=TRUE))
  ...
  print(xtable(df_matched, type = "latex"), hline.after = 0:nrow(df_matched), 
        file = paste0("psm/", hyp_name,"-matched-summary.tex"))
  
  print(xtable(df_unmatched, type = "latex"), hline.after = 0:nrow(df_unmatched), 
        file = paste0("psm/", hyp_name,"-unmatched-summary.tex"))
  ...
  print(xtable(samplesize_df, type = "latex"), file = paste0("psm/", hyp_name, "-sample-size.tex"))
  ...
  att_tab <- left_join(att, inf_names) %>%
    select(name, estimate, std.error, statistic, p.value, conf.low, conf.high) %>%
    rename(Outcome = name,
           est = estimate) %>%
      map_df(rev) %>%
    ...
    select(Outcome, Estimate, p.value, std.error) %>%
    rename("p-value" = p.value,
           "Std. Error" = std.error) 
  
  print(xtable(att_tab,  type = "latex"), include.rownames = FALSE, file = paste0("psm/", hyp_name, "-att.tex"))
...
# generate balance tables before and after matching
df3_matched <- mod3_summary$sum.matched
rownames(df3_matched) <- sapply(rownames(mod3_summary$sum.matched),
                                               function(x)
                                                filter(covar_names, var==x) %>%
                                                 select(name) %>%
                                                 as.character())
...
df3_unmatched <- mod3_summary$sum.all
rownames(df3_unmatched) <- sapply(rownames(mod3_summary$sum.all),
                                               function(x)
                                                filter(covar_names, var==x) %>%
                                                 select(name) %>%
                                                 as.character())

print(xtable(df3_matched, type = "latex"), hline.after = 0:nrow(df3_matched), include.rownames = TRUE, file = "psm/h3-matched-summary.tex")

print(xtable(df3_unmatched, type = "latex"), hline.after = 0:nrow(df3_unmatched), include.rownames = TRUE, file = "psm/h3-unmatched-summary.tex")


# Sample sizes before and after matching (need to regenerate summary using bal.table to get cluster breakdowns)
mod3_summary<-bal.tab(is_PE ~  user_created + followers + ever_verified + post_count + rt_pct + url_pct + morning_poster + afternoon_poster + evening_poster + night_poster + is_unif, data = df, weights=h3_weights, cluster="Community", stats=c("m", "ks"), abs=TRUE, cluster.summary=TRUE, estimand="ATT") 

samplesize_df3<-rbind(mod3_summary[[1]]$`Anti-vaccine`$Observations,
      mod3_summary[[1]]$`Pro-vaccine`$Observations,
      mod3_summary[[3]]) %>%
  data.frame() %>%
  rename("Perceived non-experts"="FALSE.",
         "Perceived experts"="TRUE.") %>%
  cbind(" "=c("Unmatched (Anti-vaccine)",
          "Matched (Anti-vaccine)",
          "Unmatched (Pro-vaccine)",
          "Matched (Pro-vaccine)",
          "Unmatched (All users)",
          "Matched (All users)")) %>%
  select(" ", "Perceived non-experts", "Perceived experts") %>%
  mutate_all(~as.character(.x))


print(xtable(samplesize_df3, type = "latex"), include.rownames=FALSE, file = "psm/h3-sample-size.tex")
...
print(xtable(att3_tab,  type = "latex"), include.rownames = FALSE, file = "psm/h3-att-diff.tex")

att3_within_tab <- left_join(att3_within, inf_names) %>%
  select(name, Community, estimate, std.error, statistic, p.value, conf.low, conf.high) %>%
  rename(Outcome = name,
         est = estimate) %>%
    map_df(rev) %>%
  mutate(Estimate = paste0(sprintf(est, fmt = '%#.2f'), " (", sprintf(conf.low, fmt = '%#.2f'), ", ", sprintf(conf.high, fmt = '%#.2f'), ")")) %>%
  mutate(Estimate = ifelse(est < 1e-2, paste0(formatC(est, format = "e", digits = 2), " (", formatC(conf.low, format = "e", digits=2), ", ", formatC(conf.high, format = "e", digits=2), ")"), Estimate)) %>%
  mutate(p.value= sprintf(p.value, fmt = '%#.3f')) %>%
  select(Outcome, Community, Estimate, p.value, std.error) %>%
  rename("p-value" = p.value,
         "Std. Error" = std.error) %>%
  arrange(Community)

print(xtable(att3_within_tab,  type = "latex"), include.rownames = FALSE, file = "psm/h3-att-within.tex")
...
```{r supp-att}
# Additional ATT plots, showing within-community ATT for PEs with subcommunity matching (testing H1 and H2)
# and the between-community difference in ATT (testing H3)
rbind(att1, att2) %>%
 ggplot() + 
  geom_vline(xintercept=0,  linetype='dotted')+
  geom_pointrange(aes(x=std_est, xmin=min_est, xmax=max_est, color=Community,
