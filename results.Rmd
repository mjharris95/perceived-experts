...
cent_values %>% 
  mutate(Community = ifelse(Community=="Anti-vaccine", "Antivaccine", Community)) %>%
  mutate(Community = ifelse(Community=="Pro-vaccine", "Provaccine", Community)) %>%
ggplot()+
  ...
  scale_fill_manual(values=c("Antivaccine"="#CC66BB", 
                             "Provaccine"="#66CC77"))+
...
samplesize_df3<-rbind(mod3_summary[[1]]$`Anti-vaccine`$Observations,
  ...
  cbind(" "=c("Unmatched (Anti-vaccine)",
          ...
          "Unmatched (All users)",
          "Matched (All users)")) %>%
  dplyr::select(" ", "Perceived non-experts", "Perceived experts") %>%
  mutate_all(~as.character(.x))
...
# Generates figure with propensity score matching results for within-community results
